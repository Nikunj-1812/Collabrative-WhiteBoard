...
export default function HomePage() {
  ...
  const socket = useSocket(BOARD_ID, user);
  ...
  useEffect(() => {
    const handleBoardSync = (payload: any) => {
      console.log("[HomePage] Received board:sync with leaderId:", payload.leaderId);
      console.log("[HomePage] Current user.id:", user.id);
      
      // Set the leader from server
      if (payload.leaderId) {
        setLeaderId(payload.leaderId);
        console.log("[HomePage] Leader set to:", payload.leaderId, "Is current user leader?", payload.leaderId === user.id);
      }
  ...
  const handleAddNote = (color: string) => {
    ...
    socket.emit("board:note:create", { boardId: BOARD_ID, note });
  };
  ...
  const handleClearAll = () => {
    ...
    if (window.confirm("Are you sure you want to clear all notes and drawings?")) {
      ...
      socket.emit("board:clear", { boardId: BOARD_ID });
    }
  };
  ...
  return (
    <main className="relative flex h-screen w-screen overflow-hidden bg-bg text-text">
      ...
      <div className="relative flex flex-1">
        ...
        <CollaboratorsPanel socket={socket} boardId={BOARD_ID} currentUserId={user.id} leaderId={leaderId || undefined} />
        <ShareDialog
          boardId={BOARD_ID}

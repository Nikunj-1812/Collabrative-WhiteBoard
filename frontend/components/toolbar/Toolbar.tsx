...
export const Toolbar = () => {
  ...
  return (
    <div className="pointer-events-auto flex flex-nowrap items-center gap-1 rounded-2xl border-2 border-gray-300 bg-white p-1.5 shadow-2xl sm:gap-1.5 sm:p-2 md:gap-2 md:p-2.5">
      {TOOLS.map((tool) => (
        <IconButton
          key={tool.id}
...
export const ToolbarWithClear = ({
  ...
  return (
    <div className="pointer-events-auto flex flex-nowrap items-center gap-1 rounded-2xl border border-border bg-surface/90 p-1 shadow-toolbar backdrop-blur sm:gap-1.5 sm:p-1.5 md:gap-2 md:p-2">
      {TOOLS.map((tool) => (
        <IconButton
          key={tool.id}
          active={activeTool === tool.id}
      ...
      {onClearAll && (
        <>
          ...
          <button
            ...
            className="pointer-events-auto h-9 w-9 sm:h-10 sm:w-10 md:h-11 md:w-11 rounded-lg flex items-center justify-center border-2 border-red-400 bg-gradient-to-br from-red-50 to-red-100 text-red-600 shadow-md hover:shadow-lg hover:border-red-500 hover:from-red-100 hover:to-red-150 active:scale-95 cursor-pointer transition-all duration-200"
          >
            <HiTrash size={20} />
          </button>
        </>
